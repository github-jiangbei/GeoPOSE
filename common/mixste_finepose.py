...
class  MixSTE2(nn.Module):
    def __init__(self, num_frame=9, num_joints=17, in_chans=2, embed_dim_ratio=32, depth=4,
                 ...
                 is_train=True, geometry_prompt_dim=0):
        ...
        self.geometry_prompt_dim = geometry_prompt_dim

        ### spatial patch embedding
        self.Spatial_patch_to_embedding = nn.Linear(in_chans + 3, embed_dim_ratio)
        if geometry_prompt_dim > 0:
            self.Geometry_prompt_to_embedding = nn.Sequential(
                nn.LayerNorm(geometry_prompt_dim),
                nn.Linear(geometry_prompt_dim, embed_dim_ratio),
                nn.GELU(),
                nn.Linear(embed_dim_ratio, embed_dim_ratio),
            )
        else:
            self.Geometry_prompt_to_embedding = None
    ...
    def STE_forward(self, x_2d, x_3d, t, xf_proj, geometry_prompt=None):
        ...
        if self.is_train:
            ...
            if self.Geometry_prompt_to_embedding is not None and geometry_prompt is not None:
                geometry_prompt = rearrange(geometry_prompt, 'b f n c -> (b f) n c')
    ...
    def forward(self, x_2d, x_3d, t, text, pre_text_tensor, geometry_prompt=None):
        if self.is_train:
            b, f, n, c = x_2d.shape
        else:
        ...
        x, time_embed = self.STE_forward(x_2d, x_3d, t, xf_proj, geometry_prompt)
