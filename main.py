...
dataset_path = '/home/hx/xs/TGPose/data/data_3d_' + args.dataset + '.npz'
if args.dataset == 'h36m':
    ...
    dataset = CustomDataset('/home/hx/xs/TGPose/data/data_2d_' + args.dataset + '_' + args.keypoints + '.npz')
else:
...
keypoints = np.load('/home/hx/xs/TGPose/data/data_2d_' + args.dataset + '_' + args.keypoints + '.npz', allow_pickle=True)
...
if not args.evaluate:
    ...
    while epoch < args.epochs:
        ...
        for cameras_train, batch_3d, batch_2d, batch_act in train_generator.next_epoch():
            ...
            # Predict 3D poses
            predicted_3d_pos, aux_outputs = model_pos_train(
                inputs_2d,
                inputs_3d,
                input_text,
            ...
            coarse_pose = aux_outputs.get('coarse_pose') if isinstance(aux_outputs, dict) else None
            if coarse_pose is not None:
                loss_3d_coarse = mpjpe(coarse_pose, inputs_3d)
            else:
        ...
        with torch.no_grad():
            ...
            if not args.no_eval:
                ...
                for cam, batch, batch_2d, batch_act in test_generator.next_epoch():
                    ...
                    inputs_2d = torch.from_numpy(batch_2d.astype('float32'))
                    cam_valid = None if cam is None else torch.from_numpy(cam.astype('float32'))
                    ...
                    if cam_valid is not None:
                        if cam_valid.shape[0] == 1:
                            cam_valid = cam_valid.repeat(inputs_2d.shape[0], 1)
                        elif cam_valid.shape[0] != inputs_2d.shape[0]:
                            raise ValueError('Camera batch size must be 1 or match prepared input batches.')
    ...
    with torch.no_grad():
        if newmodel is not None:
            ...
            if not use_trajectory_model:
                ...
                if args.evaluate == '':
                    chk_file_path = os.path.join(args.checkpoint, 'best_epoch_1_1.bin')
        ...
        for cam, batch, batch_2d, batch_act in test_generator.next_epoch():
            ...
            inputs_2d = torch.from_numpy(batch_2d.astype('float32'))
            inputs_3d = torch.from_numpy(batch.astype('float32'))
            cam = None if cam is None else torch.from_numpy(cam.astype('float32'))
            ...
            if cam is not None:
                if cam.shape[0] == 1:
                    cam_all = cam.repeat(inputs_2d.shape[0], 1)
                elif cam.shape[0] == inputs_2d.shape[0]:
                    cam_all = cam
                else:
                    raise ValueError('Camera batch size must be 1 or match prepared input batches.')
            ...
            for batch_cnt in range(total_batch):
                ...
                if (batch_cnt + 1) * bs > inputs_3d.shape[0]:
                    ...
                    cam_single = None if cam_all is None else cam_all[batch_cnt * bs:]
                else:
                    ...
                    cam_single = None if cam_all is None else cam_all[batch_cnt * bs:(batch_cnt + 1) * bs]
                ...
                cam_single_all = cam_single.repeat_interleave(t_sz * h_sz * f_sz, dim=0)
